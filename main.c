...
{
    if (size == 0) { // если у нас длина массива становится равна -1, то
        // возвращаем 0
        return 0;
    }
    return (arr[size - 1]
            + sum_array_recursive(
                      arr,
                      size - 1)); // возвращаем сумму элемента массива впозиции
    // size - 1 и рекурсивно вызванную функцию
}
int sum_array_iteration(int* arr, int size, int sum)
{
    if (size == 0) { // если длина массива становится равна 0, то возвращаем
        // результат
        return sum;
    }
    sum += arr[size - 1]; //каждый раз считаем сумму.
    return (sum_array_iteration(
            arr, size - 1, sum)); // возвращаем рекурсивно вызванную функцию
}
int fib_number_recursive(int n)
{
    if ((n == 1) || (n == 2)) { // первые 2 элемента из последовательностичисел
        // фибоначчи равны 1
        return 1;
    }
...
char num_to_string(int num, char* string)
{
    ...
    } else if (num < 0) { // если число отрицательное до добавляем - в начало
        // массива
        string[i++] = '-';
    }
    string[i++] = fabs(num % 10) + '0'; // добавление значения в массив
    string[i] = '\0'; // добавление нуль-терминатора в конец массива
    return i;
    // возвращаем i значение
}
int main()
{
    int x = -12345;
